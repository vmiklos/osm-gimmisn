...
stimmeljen az előző pontban kinyert értékekkel. (A suspicious-relations azt jelzi, hogy ez a reláció
nem felel meg egy valós világbeli teljes településnek, tehát hiányzó utcákat nem keresünk benne.)
...
- `inactive: true`: ez egy relációra adható meg, kikapcsolja az esetleg pazarló napi frissítést
  (hasznos, ha amúgy is 100%-on van a terület például).
...
- `suspicious-relations: only`: ezt a `relation-NÉV.yaml` file gyökrében lehet megadni egy
  ...
  házszámokat nem. Érvényes még a `yes` érték (alapértelmezés), valamint a `no`.
