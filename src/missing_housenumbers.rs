...
pub fn main(
    ...
    let mut relations = crate::areas::Relations::new(ctx)?;
    ...
    for result in ongoing_streets {
        ...
        let range_list = crate::util::get_housenumber_ranges(&result.1);
        let mut range_strings: Vec<&String> = range_list.iter().map(|i| i.get_number()).collect();
        range_strings.sort_by(|a, b| {
            crate::util::split_house_number(a).cmp(&crate::util::split_house_number(b))
        });
        stream.write_all(
...
pub fn py_missing_housenumbers_main(
    ...
    let mut stream = crate::context::PyAnyWrite { write: stdout };
