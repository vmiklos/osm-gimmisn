...
class TestFormatEvenOdd(unittest.TestCase):
    ...
    def test_happy(self) -> None:
        """Tests the happy path."""
        self.assertEqual(helpers.format_even_odd(["1", "2"], html=False), ["1", "2"])

    def test_only_odd(self) -> None:
        """Tests when we have odd numbers only."""
        self.assertEqual(helpers.format_even_odd(["1", "3"], html=False), ["1, 3"])

    def test_only_even(self) -> None:
        """Tests when we have even numbers only."""
        self.assertEqual(helpers.format_even_odd(["2", "4"], html=False), ["2, 4"])

    def test_html(self) -> None:
        """Tests HTML coloring."""
        self.assertEqual(helpers.format_even_odd(["2*", "4"], html=True), ['<span style="color: blue;">2</span>, 4'])
